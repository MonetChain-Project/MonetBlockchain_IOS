...
+ (instancetype)chatBoxWithVC:(UIViewController *)vc {
    return [[self alloc] initWithFrame:CGRectMake(0, DDYSCREENH, DDYSCREENW, 88) vc:vc];
}
...
#pragma mark 顶部文字输入区
- (void)setupTopView {
    [self addSubview:UIViewNew.viewBGColor(DDY_LightGray).viewSetFrame(0,0,self.ddy_w,0.6).viewAlpha(0.5)];
...
- (void)setupMidToolBar {
    _midToolBar = [[UIScrollView alloc] initWithFrame:CGRectMake(0, _topView.ddy_bottom, DDYSCREENW, 40)];
...
- (void)addBtnToMidToolBarWithItem:(NSString *)item {
    ...
    if ([item isEqualToString:FFChatBoxItemVoice]) {
        ...
        [_btnArray addObject:btn.btnImgNameN(@"icon_chat_emoticon").btnTag(105)];
        ...
        [_btnArray addObject:btn.btnImgNameN(@"icon_chat_transfer").btnTag(107)];
...
- (void)layoutToolBarBtn {
    if (_btnArray.count<9) {
        ...
        CGFloat btnW = _midToolBar.ddy_h-6;
...
- (void)handleTakePhoto:(DDYButton *)sender {
    [DDYAuthorityMaster cameraAuthSuccess:^{
        [self.currentVC presentViewController:self.cameraVC animated:YES completion:^{ }];
    } fail:^{ } alertShow:YES];
}
...
- (void)handleTransCard:(DDYButton *)sender {
    ...
    listVC.seletedUsersBlock = ^(NSMutableArray<FFUser *> *users) {
        FFUser *user = users[0];
        if ([weakSelf.delegate respondsToSelector:@selector(chatBox:sendCard:image:uid:sign:)]) {
            [weakSelf.delegate chatBox:self sendCard:user.nickName image:user.userImage uid:user.localID sign:user.sightml];
        }
    };
    [self.currentVC.navigationController pushViewController:listVC animated:YES];
}
...
- (FFChatBoxPic *)picBox {
    if (!_picBox) {
        __weak __typeof__(self) weakSelf = self;
        _picBox = [FFChatBoxPic picBoxWithCurrentVC:_currentVC];
    ...
    if (!_cameraVC) {
        ...
        _cameraVC.takePhotoBlock = ^(UIImage *image, UIViewController *vc) {
            [vc dismissViewControllerAnimated:YES completion:^{
                DDYPreviewController *vc = [[DDYPreviewController alloc] initWithImage:image finishBtnTitle:DDYLocalStr(@"Send") delegate:weakSelf];
                [weakSelf.currentVC.navigationController pushViewController:vc animated:YES];
            }];
        };
    }
    return _cameraVC;
