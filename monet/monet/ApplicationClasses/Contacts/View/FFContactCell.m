...
- (void)setupContentView
{
    _avatarView = ({
        ...
        [_avatarView addTapTarget:self action:@selector(headAction) number:1];
...
- (void)setUser:(FFUser *)user {
    ...
    _nameLabel.text =user.username ;//  user.noteName ?:
    _nameLabel.preferredMaxLayoutWidth = DDYSCREENW/3.0;
    
    if (self.type == FFBlackUserType) {
