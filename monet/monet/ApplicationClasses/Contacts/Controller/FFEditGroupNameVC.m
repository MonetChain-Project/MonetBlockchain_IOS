//
//  FFEditGroupNameVC.m
//  MonetBlockchain
//
//  Created by Megan on 2017/12/15.
//  Copyright © 2017年 MonetBlockchain Foundation All rights reserved.
//

#import "FFEditGroupNameVC.h"

@interface FFEditGroupNameVC ()<UITextFieldDelegate>
{
    UILabel     * _tips;
    UITextField * _nameField;
    NSInteger     _status;
    NSString    *_gid;
}
@end

@implementation FFEditGroupNameVC

-(instancetype)initWithChatUID:(NSString *)gid
{
    if (self = [super init]) {
        
        _gid = gid;
    }
    return self;
}

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.title = @"修改群聊名称";
    
    UIBarButtonItem *anotherButton = [[UIBarButtonItem alloc] initWithTitle:@"Save"  style:UIBarButtonItemStylePlain target:self action:@selector(saveAction)];
    self.navigationController.navigationBar.tintColor = [UIColor blackColor];
    self.navigationItem.rightBarButtonItem = anotherButton;
    
    [self observeNotification:UIKeyboardWillShowNotification];
    [self observeNotification:UIKeyboardWillHideNotification];
    [self observeNotification:UITextFieldTextDidChangeNotification];
    
}

-(void)buildUI
{
    self.view.backgroundColor = LC_RGB(245, 245, 245);
    [self.view addTapTarget:self action:@selector(tapAction)];
    
    _status = 0;
    
    _tips = [[UILabel alloc] initWithFrame:LC_RECT(10, 64, DDYSCREENW - 20, 45)];
    _tips.text = @"群聊名称";
    _tips.textColor = LC_RGB(180, 180, 180);
    _tips.font = NA_FONT(13);
    [self.view addSubview:_tips];
    
    _nameField = [[UITextField alloc] initWithFrame:LC_RECT(10, _tips.viewBottomY, DDYSCREENW - 10 * 2, 50)];
    _nameField.backgroundColor = [UIColor whiteColor];
    _nameField.font = NA_FONT(15);
    _nameField.placeholder = _groupName;
    _nameField.textColor = LC_RGB(51, 51, 51);
    _nameField.delegate = self;
    _nameField.keyboardType = UIKeyboardTypeDefault;
    [self.view addSubview:_nameField];
    
}

-(void) handleNotification:(NSNotification *)notification
{
    if ([notification is:UIKeyboardWillShowNotification]) {
        
        [self keyboardWillShow:notification];
        
    }else if ([notification is:UIKeyboardWillHideNotification]){
        
        [self keyboardWillHide:notification];
    }
    else if ([notification is:UITextFieldTextDidChangeNotification]){
        
        [self textFieldChange];
        
    }
    
}

- (void)tapAction
{
    [self.view endEditing:YES];
}

- (void)saveAction
{
    if(_status != 0)
    {
        [self loadNickData];
    }
    else
    {
        [self showText:@"不能为空!"];
        return;
    }
}

#pragma mark -
#pragma mark Responding to keyboard events

- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    
    [textField resignFirstResponder];
    
    return YES;
}

- (void)textFieldChange
{
    if (_nameField.text.length != 0)
    {
        _status = 1;
    }
    else
    {
        _status = 0;
    }
}

#pragma mark -
#pragma mark Responding to keyboard events

- (void)keyboardWillShow:(NSNotification *)notification {
    
    NSDictionary * userInfo = [notification userInfo];
    NSValue *animationDurationValue = [userInfo objectForKey:UIKeyboardAnimationDurationUserInfoKey];
    NSTimeInterval animationDuration;
    [animationDurationValue getValue:&animationDuration];
    
    [self moveInputBarWithKeyboardHeight:65 withDuration:animationDuration + 0.5];
}

- (void)keyboardWillHide:(NSNotification *)notification {
    
    NSDictionary* userInfo = [notification userInfo];
    NSValue *animationDurationValue = [userInfo objectForKey:UIKeyboardAnimationDurationUserInfoKey];
    NSTimeInterval animationDuration;
    [animationDurationValue getValue:&animationDuration];
    [self moveInputBarWithKeyboardHeight:0 withDuration:animationDuration+0.2];
}

- (void) moveInputBarWithKeyboardHeight:(float)height withDuration:(NSTimeInterval)interval
{
    [UIView animateWithDuration:interval animations:^{
        
        //        [_contentScrollView setContentOffset:CGPointMake(0, height)];
    }];
}

- (void)loadNickData
{
    
    if ([NSString ddy_blankString:_nameField.text]) {
        
        MBProgressHUD *hud = [self showHudWithText:@"请输入群名称"];
        [hud hideAnimated:YES afterDelay:0.5];
        return;
    }
    
    NSDictionary * params = @{@"name":_nameField.text,
                              @"plid":_gid
                              };
    [NANetWorkRequest na_postDataWithService:@"conversation" action:@"edit_conversation" parameters:params results:^(BOOL status, NSDictionary *result) {
        
        if (status)
        {
            
            dispatch_async(dispatch_get_main_queue(), ^{
                
                [self showSuccessText:@"修改成功!"];
                [self postNotification:@"backUpGroupNameNotification" withObject:_nameField.text];
                [self.navigationController popViewControllerAnimated:YES];
            });
        }
        else
        {
            NSLog(@"===网络异常==");
        }
    }];
    
}

@end
