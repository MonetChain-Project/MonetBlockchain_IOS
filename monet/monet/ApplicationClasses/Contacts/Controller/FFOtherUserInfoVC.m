...
- (void)buildUI
{
    ...
    _sendMgs.titleLabel.font = NA_FONT(12);
...
-(void)sendMgsAction
{
    ...
    [controller chatUID:self.user.localid chatType:FFChatTypeSingle groupName:self.user.username];
    [self.navigationController pushViewController:controller animated:YES];
}
...
- (void)requestUserInfo
{
    ...
    NSDictionary * params;
    if (self.user.localID) {
        params = @{@"localid":self.user.localID};
    }
    else
    {
        params = @{@"localid":self.user.localid};
    ...
    [NANetWorkRequest na_postDataWithService:@"user" action:@"userinfo" parameters:params results:^(BOOL status, NSDictionary *result) {
