...
- (void)buildUI
{
    ...
    _headIcon.image = [UIImage imageNamed:@"icon_head_defaul"];
    [_contentView addSubview:_headIcon];
    _headIcon.userInteractionEnabled = YES;
    [_headIcon addTapTarget:self action:@selector(changeHeadAction)];
...
- (void)saveAction
{
    ...
    //     Add file data.
    NSData *data = [_headIcon.image jpgCompressionWithQuality:FF_COMPRESS maxFileSize:FF_MAX_KB];
...
{
    DQActionSheet * sheet = [[DQActionSheet alloc] initWithTitle:nil cancelButtonTitle:@"Cancel" destructiveButtonTitle:nil otherButtonTitles:@"拍照",@"相册选择",@"查看大头像", nil];
    ...
    sheet.actionBlock = ^(NSInteger clickIndex){
        ...
        if (clickIndex == 2){
            ...
            [weakSelf onBtnTapHeadAction];
        }
        else if(clickIndex < 2)
        {
            [self choseActionIndex:clickIndex];
        }
    };
...
- (void)choseActionIndex:(NSInteger)index
{
    LC_UIImagePickerViewController * pikcer = [[LC_UIImagePickerViewController alloc] init];
    
    if (index == 0){
        
        pikcer.pickerType = LCImagePickerTypeCamera;
    }
    else{
        pikcer.pickerType = LCImagePickerTypePhotoLibrary;
    }

//    __weak __typeof__ (self)weakSelf = self;
    
    pikcer.imagePickFinishedBlock = ^(LC_UIImagePickerViewController * req ,NSDictionary * imageInfo){
        UIImage *srcImage=[imageInfo objectForKey:@"UIImagePickerControllerEditedImage"] ;
        UIImage * image = [srcImage scaleToWidth:FF_MAX_WIDTH height:FF_MAX_HEIGHT isCover:NO];
        // 压缩图片
    ...
    [self presentViewController:pikcer animated:YES completion:^{
        
    }];
    
}

- (void)onBtnTapHeadAction
{
    MJPhoto * photo = [[MJPhoto alloc] init];
    if (self.pic) {
        photo.url = self.pic;
    }
    photo.srcImageView = _headIcon;
    NSMutableArray * photos  = [[NSMutableArray alloc] initWithObjects:photo,nil];
    [MJPhotoBrowser showPhotos:photos currentIndex:0];
}
